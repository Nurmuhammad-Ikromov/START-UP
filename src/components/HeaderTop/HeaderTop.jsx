import React, { useEffect, useState } from 'react';
import {
    AppBar,
    Toolbar,
    Typography,
    InputBase,
    IconButton,
    Box,
    Avatar,
    alpha,
    styled
} from '@mui/material';
import {
    Search as SearchIcon,
    Clear as ClearIcon,
    Notifications as NotificationsIcon,
    CheckCircle as CheckCircleIcon
} from '@mui/icons-material';
import { API } from '../../utils/config';
import { useDispatch } from 'react-redux';
import { setMatchedBookTitle } from '../../features/search/searchSlice';

// Custom styled components
const StyledAppBar = styled(AppBar)(({ theme }) => ({
    backgroundColor: 'transparent',
    boxShadow: 'none',
    position: 'relative',

}));

const SearchContainer = styled('div')(({ theme }) => ({
    position: 'relative',
    borderRadius: theme.shape.borderRadius,
    backgroundColor: alpha(theme.palette.common.white, 0.9),
    width: '100%',
    maxWidth: '400px',
    display: 'flex',
    alignItems: 'center',
    marginLeft: '24px'
}));

const SearchIconWrapper = styled('div')(({ theme }) => ({
    padding: theme.spacing(0, 2),
    height: '100%',
    position: 'absolute',
    pointerEvents: 'none',
    display: 'flex',
    alignItems: 'center',
    justifyContent: 'center',
    color: '#757575',
}));

const StyledInputBase = styled(InputBase)(({ theme }) => ({
    color: '#333',
    width: '100%',
    '& .MuiInputBase-input': {
        padding: theme.spacing(1, 1, 1, 0),
        paddingLeft: `calc(1em + ${theme.spacing(4)})`,
        paddingRight: '2.5rem',
        transition: theme.transitions.create('width'),
        width: '100%',
    },
}));

const HeaderTop = () => {
    const [searchValue, setSearchValue] = useState('');
    const [searchResults, setSearchResults] = useState()
    const dispatch = useDispatch();

    const handleSearchChange = (event) => {
        setSearchValue(event.target.value);
    };

    useEffect(() => {

        const delayDebounce = setTimeout(() => {
            if (searchValue.trim() !== "") {
                getSearch(searchValue);
            }
        }, 500);

        return () => clearTimeout(delayDebounce);
    }, [searchValue]);


    const getSearch = async (title) => {
        try {
            const response = await API.get(`/books/${title}`);
            console.log(response.data);

            setSearchResults(response.data);
            dispatch(setMatchedBookTitle(response.data.data))
        } catch (error) {
            console.error("Search error:", error);
        }
    };
    const clearSearch = () => {
        setSearchValue('');

    };

    return (
        <StyledAppBar position="static">
            <Toolbar sx={{ zIndex: 1 }}>

                <Box >
                    <img src="logo.svg" alt="site logo" />
                </Box>

                <SearchContainer>
                    <SearchIconWrapper>
                        <SearchIcon />
                    </SearchIconWrapper>
                    <StyledInputBase
                        placeholder="Search for any training you want "
                        inputProps={{ 'aria-label': 'search' }}
                        value={searchValue}
                        onChange={handleSearchChange}
                    />
                    {searchValue && (
                        <IconButton
                            size="small"
                            sx={{ position: 'absolute', right: 5 }}
                            onClick={clearSearch}
                        >
                            <ClearIcon fontSize="small" />
                        </IconButton>
                    )}
                </SearchContainer>


                <Box sx={{ display: 'flex', alignItems: 'center', marginLeft: "auto" }}>
                    <IconButton color="inherit">
                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path d="M9.35416 21C10.0593 21.6224 10.9855 22 12 22C13.0144 22 13.9407 21.6224 14.6458 21M18 8C18 6.4087 17.3678 4.88258 16.2426 3.75736C15.1174 2.63214 13.5913 2 12 2C10.4087 2 8.88257 2.63214 7.75735 3.75736C6.63213 4.88258 5.99999 6.4087 5.99999 8C5.99999 11.0902 5.22046 13.206 4.34965 14.6054C3.61512 15.7859 3.24785 16.3761 3.26131 16.5408C3.27622 16.7231 3.31485 16.7926 3.46176 16.9016C3.59445 17 4.19258 17 5.38884 17H18.6111C19.8074 17 20.4055 17 20.5382 16.9016C20.6851 16.7926 20.7238 16.7231 20.7387 16.5408C20.7521 16.3761 20.3849 15.7859 19.6503 14.6054C18.7795 13.206 18 11.0902 18 8Z" stroke="#151515" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" />
                            <rect x="15.5" y="0.5" width="5" height="5" rx="2.5" fill="#FF4D4F" />
                            <rect x="15.5" y="0.5" width="5" height="5" rx="2.5" stroke="#FEFEFE" />
                        </svg>


                    </IconButton>
                    <Avatar
                        sx={{
                            width: 32,
                            height: 32,
                            ml: 1,
                            bgcolor: '#7c4dff'
                        }}
                        alt="User Profile"
                        src="avatar.png" // Replace with actual user image
                    >
                        U
                    </Avatar>
                </Box>
            </Toolbar>
        </StyledAppBar>
    );
};

export default HeaderTop;